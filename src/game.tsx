...
export function initGrid(): number[][]{
    const grid = Array(4).fill(0).map(()=> new Array(4).fill(0));
  ...
  export function moveDown(grid: number[][]): number[][] {
    ...
    for (let row = 3; row >= 0; row--) {
      for (let col = 0; col < 4; col++) {
        ...
        if (rowa < 3 && newGrid[rowa+1][col] === newGrid[rowa][col] && !merged[rowa + 1][col]) {
...
export function moveUp(grid: number[][]): number[][] {
    ...
    for (let row = 0; row < 4; row++) {
      for (let col = 0; col < 4; col++) {
        ...
        if (rowa > 0 && newGrid[rowa-1][col] === newGrid[rowa][col]&& !merged[rowa - 1][col]) {
          newGrid[rowa-1][col] *= 2;
          newGrid[rowa][col] = 0;
...
export function moveRight(grid: number[][]): number[][] {
    ...
    for (let x = 3; x >= 0; x--) {
      for (let y = 0; y < 4; y++) {
        ...
        if (xa < 3 && newGrid[y][xa + 1] === newGrid[y][xa]&& !merged[y][xa + 1]) {
          newGrid[y][xa + 1] *= 2;
          newGrid[y][xa] = 0;
          merged[y][xa + 1] = true;
        }
...
export function moveLeft(grid: number[][]): number[][] {
    ...
    for (let x = 0; x < 4; x++) {
      for (let y = 0; y < 4; y++) {
        ...
        if (xa > 0 && newGrid[y][xa - 1] === newGrid[y][xa]&& !merged[y][xa - 1]) {
          newGrid[y][xa - 1] *= 2;
          newGrid[y][xa] = 0;
